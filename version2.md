...
- **An unbootable mule needs hands.** A malformed `config.txt` or a bad EEPROM write can produce a
  ...
  **The array DFU is not among them, and decision 91 does not put it back**: the agent writes array
  firmware again, but not from a resource and not inside the loop — the write is a single-use,
  digest-named, interlocked operation beside it (`ArrayFirmwareFlash`), and what the loop converges
  is `firmware.xvf3800.image`, three verified downloads. So the only brick-capable *resources* are
  still the boot-partition and EEPROM writes. The array write is brick-capable and is governed
  instead by its own interlocks and by decision 91's binding sequencing: no frame is flashed until
  the Safe Mode recovery route has been rehearsed on this project's own hardware.
...
they looked, and why they decided what they did. Ten entries: the pinned base OS image, the
pinned Immich Kiosk release, the pinned LiveKit server, the commit-pinned reSpeaker control tool,
the three commit-pinned XVF3800 DFU images (decision 91 — the firmware the fleet converges
on, the known-good fallback and the blank recovery image, each pinned at the commit that last
touched *that file* rather than at the directory, because one of these filenames has already
been republished with different bytes), the two floating NuGet packages, and the .NET LTS band.
