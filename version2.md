...
5. **Retry is also pressable at the frame** (decision 72). Whoever is standing in front of it can
   ask for another go on the touchscreen once a resource has given up, without reaching a Fleet
   Manager. It is the same command over the same reset path as rung 3's, not a second mechanism.
   It requires the browser stage, so a frame that has not yet reached the kiosk stack offers no
   button and says so — see §2.7.
...
9. **A retry the person at the frame can press** once a resource has given up (decision 72), on
   the browser stage. It resets the budget through the same path the Fleet Manager's retry uses.
   The console stage has no touch input and offers no button; it names the Fleet Manager instead,
   because an affordance that does nothing is worse than an honest instruction.
...
they looked, and why they decided what they did. Seven entries: the pinned base OS image, the
pinned Immich Kiosk release, the pinned LiveKit server, the commit-pinned reSpeaker control tool,
the two floating NuGet packages, and the .NET LTS band.
