...
**Publish configuration:** two-layer simulcast, Layer 1 at 180p and Layer 2 at 720p, both at 30 fps, both derived from the capture track via `RTCRtpSender.setParameters({ encodings: [{ scaleResolutionDownBy }] })`. LiveKit's adaptive-layer selection gives small-tile subscribers the cheap 180p layer and larger or fullscreen-promoted tiles the 720p layer. If CPU validation in guide 7 shows the publisher cannot sustain both layers, Layer 2 drops to 480p before being removed; Layer 1 stays at 180p regardless.
