...
## Publisher Architecture Decision

The project's camera architecture (see [Camera & Audio research](camera-audio.md)) was debated between two options: keep Chromium as the WebRTC publisher (capture via v4l2loopback + `getUserMedia`) or move publishing to a separate GStreamer process using the LiveKit Go SDK or WHIP, with Chromium as a pure subscriber. Chromium-as-publisher was chosen, driven by requirements around low-latency fullscreen self-view, instant local mute, self-view layout flexibility, and single-participant-per-device simplicity. Full reasoning lives in the camera-audio research doc.

This decision **strengthens the case for LiveKit over Janus**. With all SFU interaction happening through the JavaScript SDK, SDK quality becomes the dominant vendor property. `livekit-client` is substantially more polished than `janus.js`, which is also GPLv3 — a concern that grows when the client is distributed inside Chromium on every device rather than served from the SFU. The auto-reconnect, JWT auto-refresh, adaptive-layer selection, and bundled TURN properties of LiveKit all live inside the same single client the SPA uses. None of the LiveKit-specific advantages that were downgraded by the Chromium-as-publisher choice (Go SDK usefulness, WHIP support, ingress/egress services) were load-bearing in the original SFU selection.

**Publish configuration:** two-layer simulcast, Layer 1 at 180p and Layer 2 at 720p, both at **56 fps** end-to-end, both derived from the capture track via `RTCRtpSender.setParameters({ encodings: [{ scaleResolutionDownBy }] })`. Capture runs at the IMX708's native 2304×1296 @ 56 fps sensor mode, so 56 fps carries from the sensor through encode, SFU, decode, and display without framerate conversion anywhere in the pipeline. LiveKit's adaptive-layer selection gives small-tile subscribers the cheap 180p layer and larger or fullscreen-promoted tiles the 720p layer. If CPU validation in guide 7 shows the publisher cannot sustain both layers at 56 fps, the first step down is capping to 30 fps via `encodings[n].maxFramerate`; if still insufficient, Layer 2 drops to 480p before being removed. Layer 1 stays at 180p regardless.

**The Janus fallback for zero-idle-decode memory efficiency** (see "Why Janus as Fallback" above) remains valid and unchanged by this decision. That property is a function of subscriber architecture (explicit subscribe vs subscribe-by-default), not publisher architecture. If 2GB RAM validation fails because LiveKit's always-subscribed model keeps too many decoders instantiated during idle/slideshow mode, Janus is still the escape hatch — and the SPA rewrite to switch is the same size it always was, since the publisher side stays in Chromium either way.
