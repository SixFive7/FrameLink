...
- **Risk** — —
- **Notes** — **Two consumers, and one of them is the agent again** (decision 91, reversing decision 90's half of this). `dfu-util` is named in exactly one code path in the agent — `ArrayFirmwareFlash`, which is single-use, digest-named and interlocked, and which refuses outright if `/usr/bin/dfu-util` is absent — and a test asserts that the name appears nowhere else in `src/FrameLink.Agent` outside that file and this package spec. The other consumer is a person: an attended bench flash through `tools/harness/flh/flash.py`, which needs the tool on the card before anybody arrives to run it. It used to be Safe Mode recovery; this project dropped its support for Safe Mode on 2026-08-24 (decision 93), so a board that stops presenting itself over USB now goes back to the maintainer and the software has no route back from that state — `reference/xvf3800-recovery-model.md` keeps the measured record of what was dropped.
...
- **Sets** — the pinned XVF3800 DFU image present under `/var/lib/fl-agent/xvf3800/xmos_firmwares/` and matching its pinned SHA-256: the **target** firmware the fleet converges on, `usb/respeaker_xvf3800_usb_dfu_firmware_v2.1.0.bin`. A **fallback** firmware and an all-`0xFF` **recovery** erase image stood beside it until 2026-08-24 and went with the pre-flight that required them (decision 93).
...
- **dependsOn** — —
- **Value source** — fixed; the image is pinned at the full commit SHA that last touched *that file* by `src/FrameLink.Agent/Firmware/XvfFirmwarePin.cs` — one definition, `<Compile Include>`d into the Fleet Manager as well (decision 93) — and reviewed in `upstream-review.json` under `xvf-firmware-target`.
- **Risk** — — (it is a verified download; nothing here touches the array)
- **Notes** — **This is the resource decision 91 added, and it is deliberately about files rather than about firmware.** A DFU write of an unverified 933 KB image is strictly worse than no flash at all — a truncated download would be pushed onto the array with nothing complaining — so a digest-verified image is the hard prerequisite for every safe flash, and it is the half of the operation that can converge unattended on every frame in the fleet with no risk at all. The write itself is **not** a resource and never will be: see "Does not become a device resource" below. **The whole pin, as one setting.** It held three images until 2026-08-24 and the reasoning was that they were not independently useful — an array that took a bad write was to be given the blank `4mb_all_ff.bin` and then a known-good fallback, so a frame carrying one without the other had a recovery route with a hole in it. Decision 93 removed both, on measured evidence that a DFU download already erases the upgrade section before it writes, that Seeed's own documented recovery has no erase step, and that the corruption the erase image existed for is fixed in firmware from v2.0.9 and caused by a command this repository sends nowhere. The setting stays "every image this build might write", which is the same shape whether the pin names one file or four. **It is also what finally makes an offline frame able to flash**: the target is vendored into the agent binary and the other two were fetched, so the pre-flight that required them was the single reason a frame with no network could not write an image it was already carrying. **No edge on `pkg.dfu-util`**, deliberately: the image is worth having on a frame whose apt run failed, and an edge there would leave it `Blocked` at exactly the moment somebody wanted to write it. **The pin is per file, not per directory**, because `respeaker_xvf3800_usb_dfu_firmware_v2.0.10.bin` has been published *twice under one name with different bytes* (43% of 933,888 bytes differing, both answering `VERSION 2 0 10`, both `bcdDevice 020a`) — so a version string identifies nothing and a directory-level watch reports every unrelated firmware upstream adds. **Which 2.1.0** — upstream publishes three — is settled in that file's remarks: Seeed's wiki states that the unsuffixed name is the 2-channel 16 kHz build, upstream's 2.0.8 changelog *adds* the six-channel `ua-io16-6ch-sqr` profile against the `ua-io16-sqr` this project's array reports, the 2.1.0 filenames spell both departures out (`16k6ch`, `48k2ch`), and the v1 capture records this array's ALSA `Capture Channel Map` with `count 2`.
...
- **The firmware *write* itself, and decision 91 keeps it out of the graph for exactly the reasoning
  ...
  `dfu-util` starts, so it can never authorise a second *operation* — decision 93 makes one
  operation up to three writes, spending the authorisation once before the first of them; and a
  flash in progress holds off the self-update, the reconciler's reboot boundary and the bench power
  switch. Decision 91 also made a rehearsed Safe Mode recovery binding before any first flash;
  **decision 93 withdrew that along with the whole of this project's Safe Mode support**, so a board
  that stops presenting itself over USB goes back to the maintainer and the software has no route
  back from that state. **Nothing here has run on hardware.**
