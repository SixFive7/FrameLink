...
Put the FrameLink Fleet Manager into production: build its container image on your workstation, ship it to your always-on Docker host, and run it there as a Portainer stack published at `https://framelink.huisman.io` through your existing Traefik, with the call server supervised inside it and alerting wired to Home Assistant. Unlike every guide before it, **the target is your server, not the frame** — steps 2 and 3 run on your Windows workstation in Git Bash, and every other step runs over SSH on the Docker host. Four things must already be true before step 1: the name resolves to that host, Traefik is running there with a working certificate resolver, Authelia is available as a Traefik middleware, and Portainer manages that Docker endpoint. **About the captured output:** every EXPECTED OUTPUT block below is real output from a real Docker host running exactly these files — but that host is the author's workstation, not your server, so image digests, container ids, generated keys, LiveKit node ids and timestamps will differ line for line. Where a value must match rather than merely resemble, LOOK FOR says so.
...
The stack file declares this volume `external: true`, which means Compose will never create it, never adopt it and — crucially — never remove it. `docker compose down -v`, a Portainer "remove the stack" with the delete-volumes box ticked, and a stack redeployed under a different name all leave an external volume exactly where it was. The cost of that protection is this one command; everything else in the deployment is disposable and this is not.
...
**Two Traefik routers, one service, and the difference between them is the whole design.** The GUI router carries your Authelia middleware. The `/agent` router carries none, because a frame is a machine with a keypair and no browser: an interactive SSO flow in front of it would not make it safer, it would make the fleet unable to connect at all. The device route authenticates by keypair over the frozen handshake instead. Traefik's default priority is rule length, which already sorts the longer `/agent` rule first, but both priorities are stated explicitly — a fleet's connectivity should not rest on a tie-break happening to go the right way.

**What Traefik cannot carry is published directly, and the port numbers are load-bearing.** Call media is UDP; a reverse proxy terminates TCP. So `7881/tcp` and `50000-50199/udp` are published on the host one-to-one, exactly as the standalone LiveKit stack in [guide 7](7-livekit-server.md) did and [guide 8](8-webrtc-validation.md) validated. One-to-one is not a convention: LiveKit advertises its own container address, which no frame on your LAN can route to, so the connection is established outwards from LiveKit and works only because the source port a frame sees is a port the host will deliver back. Remap the range to different host ports and calls connect for nobody, with no error printed anywhere.

Three variables are worth extra care. `FRAMELINK_TRUSTED_PROXIES` must be Traefik's address on the bridge it reaches this stack from — leave it empty and every client on the internet shares one rate-limit budget on the open registration path; set it too widely and the budget becomes forgeable. `FRAMELINK_LIVEKIT_PUBLIC_URL` must be the Docker host's **LAN address**, not the public domain, because media bypasses the proxy and the frames must reach the host directly. `FRAMELINK_SUBNET`, `FRAMELINK_IPV4` and `FRAMELINK_MAC` come from your PortainerCompose repository's `config/networks.yaml` and must never be invented here, because that file is what stops two stacks claiming one subnet.
...
The commands below start a throwaway container on this stack's own bridge network and make two requests to the pinned address. That is precisely the path Traefik takes, so a success here divides the world in two: everything from the bridge inwards works, and anything still failing at `https://framelink.huisman.io` is DNS, the certificate resolver, the Authelia middleware or the firewall pinhole — never the container.
...
```bash
docker run --rm --network framelink curlimages/curl:latest -fsS http://172.16.20.10:8080/healthz
docker run --rm --network framelink curlimages/curl:latest -fsS http://172.16.20.10:8080/api/status
```
...
`ok` on the first line and `"configured":true` on the second. `"configured":false` arrives with a `problem` field saying exactly what is wrong — an unset variable or a password below the length floor — and it is not a crash: the server keeps running and every frame that connects is told the server is not set up yet, which is why a frame is often the first thing that tells you. `curl: (7) Failed to connect` means the address in the command does not match `FRAMELINK_IPV4`. `curl: (28) Operation timed out` against the right address means the container is up but not listening, and `docker logs framelink` will say why. The first run of these commands also downloads the `curlimages/curl` image, which prints a few progress lines before the output above.
...
The `nodeIP` in LiveKit's own start-up line is the container's address, and it looks wrong until you know why. LiveKit runs with external-IP discovery switched off, so it advertises the address it is genuinely on; a frame on the LAN cannot route to that, and the connection is instead established outwards from LiveKit and completes because the media ports are published one-to-one. This is the same arrangement the standalone stack in [guide 7](7-livekit-server.md) used, and it is why the port count in the second command matters more than the address in the first.

`docker port` lists every published mapping. There should be 200 UDP entries — one per port in the range — plus the two TCP ports. **A smaller number is not automatically a fault:** a Docker host may have a handful of ports inside the range reserved by the operating system, and 198 of 200 is a fleet that calls perfectly. A number in the single digits means the range failed to publish, and calls will connect for nobody.
...
`starting with an operator password configured`, then `installed at`, then `started as pid`, then LiveKit's own two lines with `rtc.portICERange: [50000, 50199]`. On any start after the first, the `installed at` line is **absent** and that is the correct, idempotent behaviour — the binary was already there and verified. A line reading `refused: the download does not match the checksum upstream published` means the release has been tampered with or the pin is stale, and the Fleet Manager will not run it; everything except calling keeps working. `could not be downloaded — this server has no route to GitHub` is the same outcome from a network problem. The `198` is this host's port count and yours may differ by a few, for the reason given above; if it is `0`, the whole range failed to publish and something else on the host already holds those ports.

![ACHIEVED](https://img.shields.io/badge/🏆-ACHIEVED-228b22?style=flat-square)

The call server has been fetched, checked against its published checksum, configured with a secret only this server holds, started, and given the network ports call audio and video need — and [guide 7](7-livekit-server.md)'s separate LiveKit stack is now redundant.
