...
The channel also runs the other way, and it matters. After every hangup the app sends `{"event":"call-end"}`, and the daemon responds by restarting `framelink-camera.service`, the camera node from [guide 6](6-camera.md), so every call acquires a freshly started camera. This is a deliberate workaround, not housekeeping: the GStreamer element that publishes the camera into PipeWire (`pipewiresink` in PipeWire `1.4.x`, the version this OS ships) treats an abrupt consumer disconnect (a browser crash, or a button press landing exactly while the camera is being opened) as a fatal error and can leave the node permanently broken while its service still reports `active`. A fresh node per call makes that failure unreachable. Upstream PipeWire fixed the underlying bug in release `1.6.0`; once this OS ships PipeWire `1.6` or newer, the restart-on-call-end behaviour can be retired from the daemon.

The open socket itself carries one more job: it is the frame's proof that the kiosk *page* is alive, and the daemon acts as the page's watchdog. Chromium keeps each page in a separate renderer process, and when that renderer dies (the kernel's out-of-memory killer executing a bloated page is the observed case), the browser survives and shows an "Aw, Snap!" error tab. To systemd everything looks fine: `chromium-kiosk.service` is still `active`, so `Restart=always` never fires, and without help the frame would show that error page forever. But a dead page means a dropped WebSocket, so the daemon watches for exactly that: if no app connection has existed for 90 seconds, it restarts `chromium-kiosk.service` (never more than once per five minutes, so a genuinely broken kiosk cannot restart-loop). Restarting the browser is deliberately the frame's first-response recovery: it frees the dead page's memory, and a memory-starved frame cannot run any other cure until that happens.
