...
The UDP range `50000-50059` is published one-to-one: host port *n* maps to container port *n* and to nothing else. That is not a convention. The call server is told to advertise this PC's own LAN address, so the media candidates it offers a frame name somewhere the frame can actually send to, but each of those candidates carries the port number the call server bound *inside* the container, so a frame sent to this PC on port `50000` reaches it only while host port `50000` is the one carrying it. Remap the range and calls connect for nobody, with no error printed anywhere, which is why the fix below is a reservation rather than a different range.
