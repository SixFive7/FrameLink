...
Turn the SPA static server, GPIO daemon, and Chromium kiosk into systemd services with the correct dependency ordering, so the Pi comes up into a working kiosk state from cold boot. Then harden for 24/7 unattended operation: memory watchdog, nightly Chromium restart, overlayfs, `/tmp` on tmpfs, volatile journald, and unattended security updates.
...
After=spa-server.service
Wants=spa-server.service

[Service]
Environment=XDG_RUNTIME_DIR=/run/user/1000
ExecStart=/usr/bin/labwc -s "chromium --ozone-platform=wayland --user-data-dir=/tmp/framelink-chromium --kiosk --noerrdialogs --disable-infobars --disable-session-crashed-bubble --no-first-run --auto-accept-camera-and-microphone-capture --enable-features=UsePipeWireCamera --autoplay-policy=no-user-gesture-required --disable-background-timer-throttling --disable-renderer-backgrounding http://localhost:8888"
Restart=always
RestartSec=5
User=framelink
...
gpio-daemon.service     (starts, listens for button presses — see guide 11)
...
A short script that monitors Chromium memory and restarts it if it exceeds a threshold:
