...
_HARDWARE_FINDINGS.append(
    {
        ...
        "measured": (
            ...
            "own default - **was called the whole fix here, and it was not; see `corrected` "
            "below.** What it did do is cut 55 boots to 6, and one further repair cycle reached "
        ...
        "corrected": (
            "**\"Was the whole fix\" is falsified, and the mechanism says why it had to be.** "
            "`repair.countdownSeconds` delays the *reboot*, not the *check*. It inserts a pause "
            "before the frame goes down and changes nothing about how early the next boot asks its "
            "questions - version2.md decision 78 says this outright of the same setting: it "
            "\"pauses before a reboot rather than changing anything after one\". So the resources "
            "were still asked at the same point in the boot, still got the same empty answers, and "
            "still counted them as drift. All that moved was the length of a cycle. Re-measured "
            "2026-08-23 from Frame #1's own user journal, which still holds the boots the system "
            "journal has since rotated away: the cascades of 08-18 (`journalctl --list-boots` idx "
            "-11..-7) and 08-19 (idx -6..-1) run at **81-84 s per boot** - 03:03:27, 03:04:49, "
            "03:06:10, 03:07:32, 03:08:56 on 08-19 - against decision 64's measured **21.0 s** "
            "cycle at countdown 0. That is the setting's whole effect: 55 boots became **6**, and "
            "6 is still a cascade. It recurred on four consecutive nights after this finding was "
            "written, and on both cascade nights `audio.wireplumber.playback-volume` reached "
            "`attempts=3`, equal to the budget - the last rung before it gives up, with no margin.\n"
            "**The real cause was that a resource asked too early reports drift**, and the fix is "
            "`MediaGraphGate`: a WirePlumber that is running but has published no audio device and "
            "no default sink is `ObservationOutcome.Unevaluable`, so no attempt is spent, nothing "
            "is acted on, and nothing reboots. `audio.wireplumber.playback-volume` and "
            "`camera.pipewire-node.framelink-cam` are behind it. The window it covers is measured "
            "on the same journal: `wireplumber.service` started 03:03:27.642 and its device "
            "monitors were still loading at 03:03:31.049, 3.4 s later, while the session bus that "
            "opens `UserSessionGate` had existed since 03:03:27.571.\n"
            "**What this finding still establishes, unchanged:** @DEFAULT_AUDIO_SINK@ is the right "
            "way to address the sink; the refusal was about *when* the question was asked; and "
            "`default-routes` appearing after the first successful `wpctl set-volume` confirms "
            "decision 80. Only the sentence naming the countdown as the fix is withdrawn. **The "
            "gate has not run on hardware** - it is carried by the suite alone."
        ),
        "answer": (
            ...
            "boot for the first time in this project's history, and PCM,1 reads the same. **The "
            "countdown was not what made that true and could not have been** - see `corrected`."
        ),
        "rulesOut": (
        ...
        "recordedIn": (
            ...
            "gitignored, so these paths are local-only evidence. **The correction above is "
            "recorded in src/FrameLink.Agent/Resources/MediaGraphGate.cs**, with the two tests "
            "that fail without it - AgentAudioTests."
            "A_wireplumber_with_no_default_sink_yet_is_not_a_frame_that_is_too_quiet and "
            "AgentCameraTests.A_wireplumber_that_has_not_built_its_graph_yet_is_not_a_missing_camera "
            "- and its evidence is Frame #1's user journal, which is on the frame and not in this "
            "repository"
        ),
    }
)
