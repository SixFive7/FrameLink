...
/// frames are told to dial which this host is not on is called out rather than left silent.
/// Everything past the container — published ports, host firewall, the household router — is
/// invisible to the check and is invisible here too.
/// </para>
/// </remarks>
public sealed class LiveKitMediaProbeTests
{
    ...
    [Fact]
    public void The_container_on_its_own_bridge_is_the_silent_case_and_is_no_longer_silent()
    {
        // §3.8 puts this container on its own /24 bridge with a pinned private IPv4, and §3.7 sets
        // use_external_ip: false — so LiveKit advertises the address it is locally on. Signalling
        // arrives through the published port and works; the ICE candidates name 172.16.14.3, which
        // no frame on the house network can route to. This is the deployment's shape, not a
        // hypothetical, and it is precisely the failure that looks like a call problem.
        var host = new FakeHostSockets
        {
            Tcp = { 7880, 7881 },
            Addresses = { Bridge },
        ...
        Assert.True(check.AdvertisedAddressChecked);
        Assert.False(check.AdvertisedAddressIsLocal);
        Assert.Contains(
            check.Findings,
            finding => finding.Contains("not an address this server is on", StringComparison.Ordinal));
    }
    ...
    public void A_named_host_is_reported_unchecked_rather_than_resolved(string publicUrl)
    {
        ...
        var check = LiveKitMediaProbe.Inspect(Bundled(publicUrl), serverRunning: true, host);

        Assert.False(check.AdvertisedAddressChecked);
        Assert.True(check.AdvertisedAddressIsLocal);
        Assert.Empty(check.Findings);
    }
