...
|---|---|---|
| [Immich Kiosk](https://github.com/damongolding/immich-kiosk) | AGPL-3.0 | **v2:** the agent fetches the pinned release archive `immich-kiosk_Linux_arm64.tar.gz` from the publisher's own GitHub release, verifies it against the SHA-256 upstream publishes beside it, unpacks the single executable and supervises it as a child process — the pin lives in `src/FrameLink.Agent/Kiosk/KioskRelease.cs` and its review in `upstream-review.json`. **v1:** pulled as the published container image pinned in `deploy/immich-kiosk/compose.yaml`. Fetching from upstream rather than redistributing is deliberate in both: it keeps AGPL source-offer obligations with the publisher, off this project and off every self-hoster. |
| [LiveKit server](https://github.com/livekit/livekit) | Apache-2.0 | **v2:** the Fleet Manager fetches the pinned release archive for its own architecture (`livekit_<version>_linux_amd64.tar.gz` or the arm64 equivalent) from the publisher's own GitHub release, verifies it against the SHA-256 upstream publishes in `checksums.txt`, unpacks the single `livekit-server` executable and supervises it as a child process — the pin lives in `src/FrameLink.Control/LiveKit/LiveKitRelease.cs` and its review in `upstream-review.json`. **v1:** pulled as the published container image pinned in [guide 7](docs/7-livekit-server.md). Apache-2.0 would permit redistribution, unlike Immich Kiosk's AGPL-3.0 above; what rules it out here is size, since the two Linux binaries are 53.4 MB and 49.9 MB against a 1.35 MB agent. |
| [reSpeaker XVF3800 host control tool](https://github.com/respeaker/reSpeaker_XVF3800_USB_4MIC_ARRAY) (`xvf_host` and five sidecar files) | **None stated by Seeed** — the repository carries no licence file at all, so default copyright applies. The tool itself appears to be built from [XMOS `host_xvf_control`](https://github.com/xmos/host_xvf_control), which is published under the proprietary XCORE VOCALFUSION LICENCE. | **v2:** the agent fetches six files — `xvf_host`, `libcommand_map.so`, `libdevice_i2c.so`, `libdevice_usb.so`, `dfu_cmds.yaml`, `transport_config.yaml` — from `raw.githubusercontent.com` URLs carrying a full commit SHA, verifies each against a SHA-256 measured at review time, and installs them under `/var/lib/fl-agent`; the pin lives in `src/FrameLink.Agent/Resources/XvfHostRelease.cs` and its review in `upstream-review.json`. Upstream publishes no releases and no tags, so the commit SHA is what makes the URL content-addressed. **v1:** obtained by an unpinned `git clone --depth 1` of the whole repository in [guide 4](docs/4-audio-configuration.md). **Fetching rather than redistributing is not a preference here, it is the only lawful option**: Seeed grant nothing, and the XMOS terms forbid making the software available to a third party "on a standalone basis" while expressly permitting devices to be distributed with it installed. No byte of it is in this repository and none ever will be. FrameLink is **not** an XMOS Licensee and does not build from XMOS source — see Appendix A decision 63 of the build specification for why that matters. |
